  ...
  Widget build(BuildContext context) {
    return Scaffold(
      body: Container(
        ...
        child: FutureBuilder(
            future:
                CrewDatabaseService(uid: FirebaseAuth.instance.currentUser!.uid)
                    .getCrewUserDetails(),
            builder: (context, snapshot) {
              if (snapshot.connectionState == ConnectionState.waiting) {
                print("enter first if");
                if (snapshot.hasData) {
                  print("enter snapshot has data if");
                              ...
                              onPressed: () async {
                                ...
                                if (squadName.isNotEmpty) {
                                  ...
                                  ScaffoldMessenger.of(context).showSnackBar(
                                    const SnackBar(
                                      content:
                                          Text('Squad name cannot be empty.'),
                                      duration: Duration(seconds: 2),
                                    ),
                                  );
                                }
                  ...
                  print("enter else teina");
                  return Center(
                    child: CircularProgressIndicator(),
                  );
                }
              } else {
                print("enter else teina2");
                return Center(
                  child: CircularProgressIndicator(),
                );
              }
